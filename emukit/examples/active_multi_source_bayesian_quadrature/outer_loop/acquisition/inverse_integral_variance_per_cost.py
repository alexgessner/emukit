...
class MultiSourceInverseIntegralVariancePerCost(MultiSourceSquaredCorrelation):
    """
    Acquisition for the inverse integrated variance of the highest fidelity per cost of the to-be-evaluated
    ...
    def __init__(self, model, cost_functions):
        ...
        """
        super(MultiSourceInverseIntegralVariancePerCost, self).__init__(model)
    ...
    def evaluate(self, location, levels=None):
        ...
        return 1. / (1. - np.reshape(self._corr2(x=x), (-1, levels.size), order='F'))/ \
    ...
    def _compute_gradient_w_cost(self, location, levels):
        ...
        grad = (drho2/(1.-rho2) - dc/c)/(c * (1. - rho2))
